...
def _needs_daemon(source: str) -> bool:
    """Blocks that build the facade or a Daemon need a running daemon + paired browser.
    ...
    """
    return "ApplePasswords(" in source or "Daemon(" in source
...
def test_docs_examples(example: CodeExample, eval_example: EvalExample) -> None:
    ...
    # S105/S106 don't either (examples show obviously-fake passwords).
    eval_example.set_config(line_length=100, ruff_ignore=["T201", "S105", "S106"])

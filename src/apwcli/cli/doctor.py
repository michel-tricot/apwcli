...
def doctor() -> None:
    ...
    # 1. Platform — everything below is macOS-only.
    is_mac = sys.platform == "darwin"
    _line(is_mac, "platform", "macOS" if is_mac else sys.platform, "apwcli requires macOS")
    if not is_mac:
        raise typer.Exit(1)

    # 2. A launch-constraint-approved browser to host the extension.
    ...
    # 3. Apple's native-messaging manifest (points at the keychain helper).
    ...
    # 4. The installed iCloud Passwords extension (source of the loaded copy).
    ...
    # 5. Daemon / bridge / pairing (does not auto-start).
